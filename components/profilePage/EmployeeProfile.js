...
const EmployeeProfile = ({ empId }) => {
  ...
  const handelAllEmployees = async (e) => {
    try {
      ...
      let response = await getEmployeeById(localStorage.getItem("id"), filters);
          ...
          <div className="w-full max-w-3xl p-6 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <form className="space-y-7">
              ...
              <div className="flex justify-between">
                ...
                <div>
                  <img className="rounded mr-10" src={data.employeeImgUrl} width={200} />
                </div>
              </div>
            </form>
          </div>
