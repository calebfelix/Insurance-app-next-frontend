...
import { getAllTax } from "@/services/tax/getAllTax";
...
const AllTax = () => {
  ...
  const handleUpdate = (d) => {
    try {
      console.log(d);
      setTaxPercentage(d.taxPercentage);
      setId(d.id);
      setShow((prev) => true);
    } catch (error) {
  ...
  const updateSend = async () => {
    try {
      const res = await UpdateTax(taxPercentage, id);
      console.log(res);
      if (res.data === "tax updated sucessfully") {
        handleClose();
        handelAllTax();
